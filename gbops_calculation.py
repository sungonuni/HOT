...
from utils.analysis_calculator import BOPsCounter
...
def main_worker(rank, parsed_args):    
    ...
    if args.LoRA or args.LoRA_all:
        lora_config = LoraConfig(
            ...
            target_modules=target_module_list,  # LoRA를 적용할 레이어
            lora_dropout=0.1,        # LoRA dropout
            bias="none",             # bias 파라미터 학습 여부
        )
    ...
    gbops_counter = BOPsCounter(args.transform_scheme)

    gbops_counter.start_counting(model)
    dummy_input = torch.randn(1, 3, 224, 224).to('cuda')

    _ = model(dummy_input)

    gbops_counter.stop_counting()

    results = gbops_counter.get_total_gbops()
    print(f"Run Name: {args.RUN_NAME}")
    print(f"Total Forward gbops: {results['total_forward_gbops']/1e9:.2f}")
    print(f"Total Backward gbops: {results['total_backward_gbops']/1e9:.2f}")
    print(f"Total gbops: {results['total_gbops']/1e9:.2f}")
